...
class GroupShardedStage2(nn.Layer):
                ...
                if self._grad_reduced[index]:
                    ...
                    def cleanup():
                        if dst_rank != self._rank:
                            ...
                            if self.use_main_grad:
                                tmp_grad = param.main_grad.clone().cpu()
                            else:
            ...
            if self.use_main_grad:
                logger_.info(
                    "====== BF16 GradStorage size: {:.2f}M parameters, Model size {:.2f}M parameters ======".format(
                        rank_buffer_size[Type.bf16.value] / 2**18,
                        model_size / 2**19,
                    )
                )
            else:
    ...
    def _dp_allreduce(self):
        ...
        if self._dp_group and self._dp_group.nranks > 1:
            ...
            for param in self._trainable_params:
                if param.name in self._param_grads and (
                    (param.grad is not None)
                    or (self.use_main_grad and param.main_grad is not None)
                ):
                    dst_rank = self._trainable_param2rank[param.name]
                    if dst_rank == self._rank:
                        dist.all_reduce(
